...
{
    t_paquete *paquete_IO = crear_paquete_con_codigo_de_operacion(ENVIAR_INTERFAZ);
    serializar_IO_instruccion(paquete_IO,pcb_actual,unidades_de_trabajo, interfaz);
...
void serializar_IO_instruccion(t_paquete *paquete,t_pcb *pcb_actual ,uint32_t unidades_de_trabajo, char *interfaz)
{
    ...
    size_t tam_registros = sizeof(uint32_t) +
                           sizeof(uint8_t) * 4 +
                           sizeof(uint32_t) * 6;
    int buffer_size = sizeof(uint32_t) + sizeof(uint32_t) + interfaz_length 
                    sizeof(uint32_t) +
